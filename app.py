...
class DocumentAnalyzer:
    def __init__(self, api_key=None):
        ...
        if not api_key:
            raise ValueError("❌ OPENAI_API_KEY missing — set in Streamlit secrets")
    ...
    def extract_text_with_ocr(self, pdf_file):
        ...
        try:
            ...
            print(f"[DEBUG] extract_text_with_ocr error: {e}")
            return ""
    ...
    def convert_pdf_to_images(self, pdf_file):
        """Convert first page of PDF to base64 JPEG using PyMuPDF (no Poppler needed)"""
        try:
            import fitz  # PyMuPDF
        ...
        try:
            ...
            if file_type == "pdf":
                ...
                if text and len(text.strip()) > 20:
                    ...
                    base64_image = self.convert_pdf_to_images(file)
                    if not base64_image:
                        return {"error": "Unable to extract text or convert PDF to image. File may be corrupted."}
                    
                    user_content = [
                        {"type": "text", "text": f"{analysis_prompt}\n\nThis is a scanned document. Please analyze the image carefully and extract all visible information. Return ONLY valid JSON."},
